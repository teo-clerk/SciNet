...
terminal it was typed into.
    ...
    uv run python ../scripts/stop.py            # stop everything
    uv run python ../scripts/stop.py --dry-run  # show what would be stopped
"""
...
def workers() -> set[int]:
    ...
    for pid, cmdline in _process_table():
        if pid in protected:
            continue
        # A shell whose *arguments* mention the runner is not the runner. Only
        # a python interpreter actually running the module counts.
        if WORKER_PATTERN.search(cmdline) and "python" in cmdline.lower():
            found.add(pid)
    return found
